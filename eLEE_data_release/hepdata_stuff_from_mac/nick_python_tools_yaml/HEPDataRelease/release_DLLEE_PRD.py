...
# Code snippet
code_snippet = heptype.AdditionalResource(
    location="CalculateChiSquareCNP.py",
    description="Code snippet for calculating the CNP chi square values reported in Table II of arXiv:2110.14080 using this data release",
    license=None,
)
...
submission = heptype.Submission(
    additional_resources=[code_snippet],
...
nue_bkg_file = convert.expect_and_bins(
    data_file="nue_1e1p_constr_pred.txt",
    data_name=r"1e1p Sample Total Background Prediction (Constrained)",
    bin_file="nue_1e1p_bins.txt",
    bin_name=r"$E_\nu$",
)

orig_nue_bkg_file = heptype.AdditionalResource(
    location="nue_1e1p_constr_pred.txt",
    description="1D array of predicted total background events per reconstructed neutrino energy bin after the 1mu1p constraint.",
    license=None,
)

# NuE lee prediction
nue_lee_file = convert.expect_and_bins(
    data_file="nue_1e1p_constr_pred_lee.txt",
    data_name=r"1e1p Sample Total Background + LEE Prediction (Constrained)",
    bin_file="nue_1e1p_bins.txt",
    bin_name=r"$E_\nu$",
)

orig_nue_lee_file = heptype.AdditionalResource(
    location="nue_1e1p_constr_pred_lee.txt",
    description="1D array of predicted total background + LEE events per reconstructed neutrino energy bin after the 1mu1p constraint.",
    license=None,
)

# NuMu Background Fit
numu_bkg_file = convert.expect_and_bins(
    data_file="nue_1e1p_numu_bkg_fit.txt",
...
orig_numu_bkg_file = heptype.AdditionalResource(
    location="nue_1e1p_numu_bkg_fit.txt",
...
data_files.append(heptype.InlineResource(
    name="NuE data and background (+ LEE) constrained prediction",
...
##########################
# Unconstrained Prediciton
##########################

# NuE background prediction
nue_bkg_file = convert.expect_and_bins(
    data_file="nue_1e1p_nom_pred.txt",
    data_name=r"1e1p Sample Total Background Prediction (Unconstrained)",
    bin_file="nue_1e1p_bins.txt",
    bin_name=r"$E_\nu$",
)

orig_nue_bkg_file = heptype.AdditionalResource(
    location="nue_1e1p_nom_pred.txt",
    description="1D array of predicted total background events per reconstructed neutrino energy bin before the 1mu1p constraint.",
    license=None,
)

# NuE lee prediction
nue_lee_file = convert.expect_and_bins(
    data_file="nue_1e1p_nom_pred_lee.txt",
    data_name=r"1e1p Sample Total Background + LEE Prediction (Unonstrained)",
    bin_file="nue_1e1p_bins.txt",
    bin_name=r"$E_\nu$",
)

orig_nue_lee_file = heptype.AdditionalResource(
    location="nue_1e1p_nom_pred_lee.txt",
    description="1D array of predicted total background + LEE events per reconstructed neutrino energy bin before the 1mu1p constraint.",
    license=None,
)

data_files.append(heptype.InlineResource(
    name="NuE data and background (+ LEE) unconstrained prediction",
    description="Observed NuE data and background (+ LEE) prediction, including the muon neutrino background prediction from the empirical fit, for arXiv:2110.14080. The prediction does not incorporate the constraint from the 1mu1p sample",
    keywords=energy_keywords,
    independent_variables=nue_data_file.independent_variables,
    dependent_variables=(nue_data_file.dependent_variables + nue_bkg_file.dependent_variables + nue_lee_file.dependent_variables + numu_bkg_file.dependent_variables),
    data_license=None,
...
orig_nue_bkg_frac_cov_file = heptype.AdditionalResource(
    location="nue_1e1p_cov_constr.txt",
...
nue_lee_frac_cov_file = convert.cov_and_bins(
    data_file="nue_1e1p_cov_constr_lee.txt",
...
orig_nue_lee_frac_cov_file = heptype.AdditionalResource(
    location="nue_1e1p_cov_constr_lee.txt",
...
data_files.append(heptype.InlineResource(
    name="NuE background unconstrained fractional covariance matrix",
...
data_files.append(heptype.InlineResource(
    name="NuE background+LEE unconstrained fractional covariance matrix",
